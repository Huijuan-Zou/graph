...
public class AndPredicateTest{
  ...
  public void testAccept() {
    Predicate<Vertex> labelNotAC = 
        new AndPredicate<Vertex>(labelNotA, labelNotC); 
    for (Vertex vertex : directedGraph.getVertices()) {
      if (vertex.getLabel().equals("A")) {
        assertFalse(labelNotAC.accept(vertex));
      } else if (vertex.getLabel().equals("C")) {
        assertFalse(labelNotAC.accept(vertex));
      } else {
        assertTrue(labelNotAC.accept(vertex));
      }
    }
  }
  ...
  public void testNullInput() {
    Predicate<Vertex> andPredicate = 
        new AndPredicate<Vertex>(null, labelNotC);  
  }

  @Test (expected = IllegalArgumentException.class)
  public void testNullInput_andPredicate_twoNull() {
    Predicate<Vertex> andPredicate = 
        new AndPredicate<Vertex>(null, null);
  }

  @Test (expected = IllegalArgumentException.class)
  public void testNullInput_andPredicate_oneNull() {
    Predicate<Vertex> andPredicate = 
        new AndPredicate<Vertex>(labelNotA, null);
  }

  @Test 
  public void testAccept_null_emptyInput() {
    Predicate andLabel = 
        new AndPredicate(labelNotA, labelNotC);
    assertFalse(andLabel.accept(null));
    assertFalse(andLabel.accept(""));
  }
}